...
# Load the CSV file
csv_file_path = '../../bfs/data/BFS_DFC_omp_runtime-results.csv'
...
# y_values_1 = safe_divide(data.iloc[:, 4], data.iloc[:, 5]) / 1000000  # 4th divided by 8th column, GraphBLAST
y_values_2 = safe_divide(data.iloc[:, 4], data.iloc[:, 5]) / 1000000  # 4th divided by 9th column, ALBBA
# y_values_3 = safe_divide(data.iloc[:, 4], data.iloc[:, 5]) / 1000000  # 4th divided by 10th column, Enterprise
...
plt.savefig("../../bfs/figures/perf-bar.pdf", dpi = 300)
